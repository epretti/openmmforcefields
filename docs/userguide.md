...
This repository contains force fields for use with the OpenMM [`ForceField`](https://docs.openmm.org/latest/api-python/generated/openmm.app.forcefield.ForceField.html) class for parameterizing biomolecular systems.
If you're not familiar with this approach to applying parameters to biomolecular systems, please see the [OpenMM User Guide](https://docs.openmm.org/latest/userguide/application/02_running_sims.html).
Note that a selection of standard force fields from the Amber and CHARMM force field families is already bundled with the main OpenMM release, as also described [in the User Guide](https://docs.openmm.org/latest/userguide/application/02_running_sims.html#force-fields).
Those force fields should be identical to the versions distributed with OpenMMForceFields but can be used directly without installing this package.  However, OpenMMForceFields provides a larger set of force fields from these families.

### Using the Amber force fields

Once installed, the Amber force fields will be registered in the `amber/` relative path searched by [`openmm.app.ForceField`](http://docs.openmm.org/latest/api-python/generated/openmm.app.forcefield.ForceField.html).

For example, to specify the [`ff19SB`](https://doi.org/10.1021/acs.jctc.9b00591) protein force field (corresponding to the force field loaded by LEaP with `leaprc.protein.ff19SB`):
```python
forcefield = ForceField("amber/protein.ff19SB.xml")
```
or to specifically access the older (now outdated) [`ff99SBildn`](https://doi.org/10.1002/prot.22711) force field converted from `oldff/leaprc.ff99SBildn`:
```python
...
The OpenMMForceFields package includes a [residue template generator](https://docs.openmm.org/latest/userguide/application/06_creating_ffs.html#adding-residue-template-generators) for the OpenMM [`ForceField`](https://docs.openmm.org/latest/api-python/generated/openmm.app.forcefield.ForceField.html) class that automatically generates OpenMM residue templates for small molecules lacking parameters using [GAFF](https://ambermd.org/antechamber/gaff.html) versions 1 or 2.

### Cheminformatics toolkits

The [OpenFF Toolkit](https://github.com/openforcefield/openff-toolkit) is used to provide an interface with cheminformatics toolkits to interact with [Antechamber](https://ambermd.org/antechamber/ac.html) from the [AmberTools](https://ambermd.org/AmberTools.php) package to generate parameters for small molecules.
By default, the OpenFF Toolkit will make use of the free and open source [RDKit cheminformatics toolkit](https://www.rdkit.org/) that is installed automatically, but will optionally use the [OpenEye toolkit](https://docs.eyesopen.com/toolkits/python/index.html) if it is installed and licensed.
...
Generation of OpenMM-compatible parameters for small molecules encountered in an OpenMM [`Topology`](https://docs.openmm.org/latest/api-python/generated/openmm.app.topology.Topology.html) is handled through `openmmforcefields.generators.GAFFTemplateGenerator`.
Because the OpenMM [`Topology`](https://docs.openmm.org/latest/api-python/generated/openmm.app.topology.Topology.html) object used by the OpenMM [`ForceField`](https://docs.openmm.org/latest/api-python/generated/openmm.app.forcefield.ForceField.html) class does not know the precise chemical identity of molecules represented in the topology—which contains only elements and bonds between them, without stereochemical or bond order information—it is necessary to instruct `GAFFTemplateGenerator` which small molecules it will encounter in processing the `Topology` object ahead of time; it then matches these by element and bond pattern.

### Specifying molecules

To do this, it is necessary to specify one or more [`openff.toolkit.Molecule`](https://docs.openforcefield.org/projects/toolkit/en/stable/api/generated/openff.toolkit.topology.Molecule.html) objects which can easily be created from many different representations of small molecules, including SMILES strings and common molecule storage formats.
There are many ways to create an OpenFF [`Molecule`](https://docs.openforcefield.org/projects/toolkit/en/stable/api/generated/openff.toolkit.topology.Molecule.html) object: see the [OpenFF Toolkit documentation](https://docs.openforcefield.org/projects/toolkit/en/stable/users/molecule_cookbook.html) for more details.

### Partial charges for small molecules

If the provided molecule(s) contain nonzero (user-specified) partial charges (stored in the [`Molecule.partial_charges`](https://docs.openforcefield.org/projects/toolkit/en/stable/api/generated/openff.toolkit.topology.Molecule.html#openff.toolkit.topology.Molecule.partial_charges) attribute), those partial charges will be used.
If they do _not_ contain partial charges, [Antechamber](https://ambermd.org/antechamber/ac.html) (which uses the `sqm` semiempirical quantum chemical package) is used to assign AM1-BCC charges (`antechamber -c bcc`). The conformers used in charge assignment are generated by RDKit.
...
# Create an OpenMM ForceField object with Amber ff19SB and TIP3P-FB
...
The OpenMMForceFields package includes a [residue template generator](https://docs.openmm.org/latest/userguide/application/06_creating_ffs.html#adding-residue-template-generators) for the OpenMM [`ForceField`](https://docs.openmm.org/latest/api-python/generated/openmm.app.forcefield.ForceField.html) class that automatically generates OpenMM residue templates for small molecules lacking parameters using the [Open Force Field Initiative](https://openforcefield.org/) [SMIRNOFF](https://openforcefield.github.io/standards/standards/smirnoff/) small molecule force fields.
This includes the [`openff-1.x.y` ("Parsley")](https://doi.org/10.1021/acs.jctc.1c00571) and [`openff-2.x.y` ("Sage")](https://doi.org/10.1021/acs.jctc.3c00039) small molecule force field lines, including the [most recent force field in each line](https://github.com/openforcefield/openff-forcefields).
...
Like `GAFFTemplateGenerator`, when user-specified partial charges are not present, partial charges will be assigned automatically.  For SMIRNOFF force fields, the charges are assigned by the OpenFF software using [OpenFF NAGL](https://github.com/openforcefield/openff-nagl) or according to the [AM1-BCC settings of the SMIRNOFF specification](https://openforcefield.github.io/standards/standards/smirnoff/#toolkitam1bcc-temporary-support-for-toolkit-based-am1-bcc-partial-charges) (depending on the force field in use).
...
Here's an example that uses GAFF 2.2.20 along with the Amber `ff19SB` force field and TIP3P-FB water model to generate an OpenMM [`System`](https://docs.openmm.org/latest/api-python/generated/openmm.openmm.System.html) from an OpenMM [`Topology`](https://docs.openmm.org/latest/api-python/generated/openmm.app.topology.Topology.html):
```python
